...
namespace MadokaMod.Cards.Template
{
    public static class CardIndexGenerator
    {
        private static readonly List<ManaColor> offColors = new List<ManaColor>() { ManaColor.Colorless };
        ...
        public static int GetUniqueIndex(CardConfig config)
        {
            ...
            */
            ...
            //Color
            int color = config.Colors.Count > 1 ? 9 : (config.Colors.Count == 0 ? (int)ManaColor.Colorless : (int)config.Colors[0]);
            id += color * 10000; 
